  ...
  as.numeric(
    unique(
      (ttm_rawrwdata %>% filter(
        Period %in% c(56,57) & Key == "rp.group1Finished"))$Sender
    )
  )
)
...
SeanR_ttm = data.frame(
  ...
  mutate(
    SeanR.Group1 = ifelse(Sender %in% c(mkt1sup,mkt1dem), T, F),
    SeanR.Group2 = ifelse(Sender %in% c(mkt2sup,mkt3dem), T, F),
    SeanR.inTTM  = ifelse((Sender %in% c(mkt1sup,mkt1dem, mkt2sup,mkt3dem)), T, F)
  )
      
    # SeanR.mkt1sup = ifelse(Sender %in% mkt1sup, T, F),
    # SeanR.mkt1dem = ifelse(Sender %in% mkt1dem, T, F),
    # SeanR.mkt2sup = ifelse(Sender %in% mkt2sup, T, F),
    # SeanR.mkt2dem = ifelse(Sender %in% mkt2dem, T, F),
    # SeanR.mkt3sup = ifelse(Sender %in% mkt3sup, T, F),
    # SeanR.mkt3dem = ifelse(Sender %in% mkt3dem, T, F),
    # SeanR.mkt4sup = ifelse(Sender %in% mkt4sup, T, F),
    # SeanR.mkt4dem = ifelse(Sender %in% mkt4dem, T, F)
